  ...
  return (
    <AppShell>
      ...
      <section className="mx-auto w-full max-w-xl rounded-2xl border border-slate-200 bg-white p-6">
        <h2 className="text-xl font-extrabold">관리자 비밀번호 변경</h2>
        <p className="mt-2 text-sm text-slate-500">데모용 로컬 저장 데이터에 반영됩니다.</p>

        <input type="password" value={pin} onChange={(event) => setPin(event.target.value)} className="mt-5 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-base" placeholder="새 비밀번호" />
